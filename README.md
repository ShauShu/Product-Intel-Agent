...
```cmd
REM 分析單一競品
curl -X POST http://localhost:8080/analyze -H "Content-Type: application/json" -d "{\"competitor\": \"五月花衛生紙\", \"session_id\": \"test-001\"}"
...
**Q：搜尋結果不夠精準怎麼辦？**  
A：修改 `agent/agent.py` 中 `RESEARCHER_PROMPT` 的搜尋指示，或在呼叫 `/analyze` 時傳入更具體的關鍵字，例如 `"五月花三層抽取式衛生紙 規格比較"` 或 `"2025 衛生紙漲價趨勢預測"` 而非單純的 `"五月花衛生紙"`。
